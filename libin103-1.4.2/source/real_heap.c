...
int real_heap_insert(real_heap_t *heap, double data) {
  ...
  tempTree = realloc(heap->tree, (real_heap_size(heap) + 1) * sizeof(double));
  if (tempTree == NULL) {
    return -1;
  }
  else {
  ...
  heap->tree[real_heap_size(heap)] = data;

  /* Heapify the tree by pushing the contents of the new node upward. */
  ipos = real_heap_size(heap);
  ppos = heap_parent(ipos);

  bool flag = false;
  if (heap->heap_type == real_MAX_HEAP) {
...
int real_heap_extract(real_heap_t *heap, double *data) {
  ...
  *data = heap->tree[0];
  ...
  save = heap->tree[real_heap_size(heap) - 1];

  if (real_heap_size(heap) - 1 > 0) {
    tempTree = realloc(heap->tree, (real_heap_size(heap)-1) * sizeof(double));
    if (tempTree == NULL) {
      return -1;
    }
  ...
  while (1) {
    ...
    if (lpos < real_heap_size(heap) && flag) {
      mpos = lpos;
    }
    else {
      mpos = ipos;
    ...
    if (rpos < real_heap_size(heap) && flag) {
      mpos = rpos;
    }
