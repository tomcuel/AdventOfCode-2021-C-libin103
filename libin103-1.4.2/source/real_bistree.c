...
static void* generic_build (const void *avl_data) {
  real_avlnode_t *new_avl_data = malloc(sizeof(real_avlnode_t));
  assert(new_avl_data!=NULL);
  new_avl_data->factor = ((real_avlnode_t *)avl_data)->factor;
  new_avl_data->hidden = ((real_avlnode_t *)avl_data)->hidden;
  new_avl_data->data = ((real_avlnode_t *)avl_data)->data;
  return (void*) new_avl_data;
}

static void generic_destroy_right(real_bistree_t *tree,
...
static void generic_rotate_left(generic_bitreenode_t **node) {
  ...
  real_avlnode_t* nodeLeft = generic_bitree_data(left);
  if (nodeLeft->factor == real_AVL_LFT_HEAVY) {
    ...
    ((real_avlnode_t *)(*node)->data)->factor = real_AVL_BALANCED;
    ((real_avlnode_t *)left->data)->factor = real_AVL_BALANCED;
    *node = left;
  }
  else {
    ...
    switch (((real_avlnode_t *)generic_bitree_data(grandchild))->factor) {

    case real_AVL_LFT_HEAVY:
      ((real_avlnode_t *)generic_bitree_data(*node))->factor = real_AVL_RGT_HEAVY;
      ((real_avlnode_t *)generic_bitree_data(left))->factor = real_AVL_BALANCED;
      break;

    case real_AVL_BALANCED:
      ((real_avlnode_t *)generic_bitree_data(*node))->factor = real_AVL_BALANCED;
      ((real_avlnode_t *)generic_bitree_data(left))->factor = real_AVL_BALANCED;
      break;

    case real_AVL_RGT_HEAVY:
      ((real_avlnode_t *)generic_bitree_data(*node))->factor = real_AVL_BALANCED;
      ((real_avlnode_t *)generic_bitree_data(left))->factor = real_AVL_LFT_HEAVY;
      break;
    }

    ((real_avlnode_t *)generic_bitree_data(grandchild))->factor = real_AVL_BALANCED;
    *node = grandchild;
  }
}
...
static void generic_rotate_right(generic_bitreenode_t **node) {
  ...
  real_avlnode_t * nodeAvl = generic_bitree_data(right);
  if (nodeAvl->factor == real_AVL_RGT_HEAVY) {
   /* Perform an RR rotation. */
    ...
    ((real_avlnode_t *)(*node)->data)->factor = real_AVL_BALANCED;
    ((real_avlnode_t *)right->data)->factor = real_AVL_BALANCED;
    *node = right;
    /* printf ("%s: RR rotation: *node=%p et right=%p\n", __FILE__, (void*)*node, (void*)right); */
  }
  else {
   ...
   case real_AVL_LFT_HEAVY:
     ((real_avlnode_t *)generic_bitree_data(*node))->factor = real_AVL_BALANCED;
     ((real_avlnode_t *)generic_bitree_data(right))->factor = real_AVL_RGT_HEAVY;
     break;

   case real_AVL_BALANCED:
     ((real_avlnode_t *)generic_bitree_data(*node))->factor = real_AVL_BALANCED;
     ((real_avlnode_t *)generic_bitree_data(right))->factor = real_AVL_BALANCED;
     break;

   case real_AVL_RGT_HEAVY:
     ((real_avlnode_t *)generic_bitree_data(*node))->factor = real_AVL_LFT_HEAVY;
     ((real_avlnode_t *)generic_bitree_data(right))->factor = real_AVL_BALANCED;
     break;
   }
...
static void generic_destroy_left(real_bistree_t *tree,
				 generic_bitreenode_t *node)
{
  ...
  if (*position != NULL) {
    ...
    /*   tree->destroy(((real_avlnode_t *)(*position)->data)->data); */
   /* } */
...
static int generic_insert(real_bistree_t *tree,
			  generic_bitreenode_t **node,
			  double data,
			  int *balanced)
{
  real_avlnode_t *avl_data;
  int cmpval;
  int retval;
  ...
  if (generic_bitree_is_eob(*node)) {
    ...
    avl_data = malloc(sizeof(real_avlnode_t));
    if (avl_data == NULL) {
      return -1;
    }
    avl_data->factor = real_AVL_BALANCED;
    avl_data->hidden = 0;
    avl_data->data = data;
    ...
    real_avlnode_t *tempNode =  generic_bitree_data(*node);
	...
	*balanced = 0;
	...
	switch (((real_avlnode_t *)generic_bitree_data(*node))->factor) {
	...
	*balanced = 0;
	...
	case real_AVL_RGT_HEAVY:
	  ...
	  *balanced = 1;
      ...
      if (!((real_avlnode_t *)generic_bitree_data(*node))->hidden) {
	...
	if (tree->destroy != NULL) {
	  ...
	  /* tree->destroy(((real_avlnode_t *)generic_bitree_data(*node))->data); */
	}

	((real_avlnode_t *)generic_bitree_data(*node))->data = data;
	((real_avlnode_t *)generic_bitree_data(*node))->hidden = 0;
...
static int generic_hide(real_bistree_t *tree,
			generic_bitreenode_t *node,
			double data)
{
  ...
  if (cmpval < 0) {
   ...
   ((real_avlnode_t *)generic_bitree_data(node))->hidden = 1;
   retval = 0;
  }
...
static bool generic_lookup(real_bistree_t *tree,
			  generic_bitreenode_t *node,
			  double data)
{
  ...
  if (cmpval < 0) {
   /* Move to the left. */
    ...
    if (!((real_avlnode_t *)generic_bitree_data(node))->hidden) {
      ...
      /* *data = ((real_avlnode_t *)generic_bitree_data(node))->data; */
      retval = true;
    }
    else {
      /* Return that the data was not found. */
...
int real_bistree_remove(real_bistree_t *tree,
			   double data)
{
...
bool real_bistree_lookup(real_bistree_t *tree,
			   double data)
{
