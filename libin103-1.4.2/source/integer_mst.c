...
int integer_mst(integer_graph_t *graph, generic_list_t **span) {
  ...
  integer_uf_t dset;
  integer_uf_init (&dset, maxIndex+1);
  ...
  *span = malloc (sizeof (generic_list_t));
  ...
  while (generic_heap_size (&heap) > 0) {
    ...
    /* Step 6 */
    if (! integer_uf_are_connected (&dset, currentWE->source, currentWE->destination)) {
      generic_list_ins_next (*span, generic_list_tail(*span), (void*) currentWE);
      integer_uf_union (&dset, currentWE->source, currentWE->destination);
    }
