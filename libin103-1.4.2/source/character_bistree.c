...
#include "character_bistree.h"
#include "bitree.h"

#define compare(a,b) (((a)<(b))?-1:((a)>(b)?1:0))

static void* generic_build (const void *avl_data) {
  character_avlnode_t *new_avl_data = malloc(sizeof(character_avlnode_t));
  assert(new_avl_data!=NULL);
  new_avl_data->factor = ((character_avlnode_t *)avl_data)->factor;
  new_avl_data->hidden = ((character_avlnode_t *)avl_data)->hidden;
  new_avl_data->data = ((character_avlnode_t *)avl_data)->data;
  return (void*) new_avl_data;
}

static void generic_destroy_right(character_bistree_t *tree,
...
static void generic_rotate_left(generic_bitreenode_t **node) {
  ...
  character_avlnode_t* nodeLeft = generic_bitree_data(left);
  if (nodeLeft->factor == character_AVL_LFT_HEAVY) {
    ...
    ((character_avlnode_t *)(*node)->data)->factor = character_AVL_BALANCED;
    ((character_avlnode_t *)left->data)->factor = character_AVL_BALANCED;
    *node = left;
  }
  else {
    ...
    switch (((character_avlnode_t *)generic_bitree_data(grandchild))->factor) {

    case character_AVL_LFT_HEAVY:
      ((character_avlnode_t *)generic_bitree_data(*node))->factor = character_AVL_RGT_HEAVY;
      ((character_avlnode_t *)generic_bitree_data(left))->factor = character_AVL_BALANCED;
      break;

    case character_AVL_BALANCED:
      ((character_avlnode_t *)generic_bitree_data(*node))->factor = character_AVL_BALANCED;
      ((character_avlnode_t *)generic_bitree_data(left))->factor = character_AVL_BALANCED;
      break;

    case character_AVL_RGT_HEAVY:
      ((character_avlnode_t *)generic_bitree_data(*node))->factor = character_AVL_BALANCED;
      ((character_avlnode_t *)generic_bitree_data(left))->factor = character_AVL_LFT_HEAVY;
      break;
    }

    ((character_avlnode_t *)generic_bitree_data(grandchild))->factor = character_AVL_BALANCED;
    *node = grandchild;
  }
}
...
static void generic_rotate_right(generic_bitreenode_t **node) {
  ...
  character_avlnode_t * nodeAvl = generic_bitree_data(right);
  if (nodeAvl->factor == character_AVL_RGT_HEAVY) {
   /* Perform an RR rotation. */
    ...
    ((character_avlnode_t *)(*node)->data)->factor = character_AVL_BALANCED;
    ((character_avlnode_t *)right->data)->factor = character_AVL_BALANCED;
    *node = right;
    /* printf ("%s: RR rotation: *node=%p et right=%p\n", __FILE__, (void*)*node, (void*)right); */
  }
  else {
   ...
   case character_AVL_LFT_HEAVY:
     ((character_avlnode_t *)generic_bitree_data(*node))->factor = character_AVL_BALANCED;
     ((character_avlnode_t *)generic_bitree_data(right))->factor = character_AVL_RGT_HEAVY;
     break;

   case character_AVL_BALANCED:
     ((character_avlnode_t *)generic_bitree_data(*node))->factor = character_AVL_BALANCED;
     ((character_avlnode_t *)generic_bitree_data(right))->factor = character_AVL_BALANCED;
     break;

   case character_AVL_RGT_HEAVY:
     ((character_avlnode_t *)generic_bitree_data(*node))->factor = character_AVL_LFT_HEAVY;
     ((character_avlnode_t *)generic_bitree_data(right))->factor = character_AVL_BALANCED;
     break;
   }
...
static void generic_destroy_left(character_bistree_t *tree,
				 generic_bitreenode_t *node)
{
  ...
  if (*position != NULL) {
    ...
    /*   tree->destroy(((character_avlnode_t *)(*position)->data)->data); */
   /* } */
...
static int generic_insert(character_bistree_t *tree,
			  generic_bitreenode_t **node,
			  char data,
			  int *balanced)
{
  character_avlnode_t *avl_data;
  int cmpval;
  int retval;
  ...
  if (generic_bitree_is_eob(*node)) {
    ...
    avl_data = malloc(sizeof(character_avlnode_t));
    if (avl_data == NULL) {
      return -1;
    }
    avl_data->factor = character_AVL_BALANCED;
    avl_data->hidden = 0;
    avl_data->data = data;
    ...
    character_avlnode_t *tempNode =  generic_bitree_data(*node);
	...
	*balanced = 0;
	...
	switch (((character_avlnode_t *)generic_bitree_data(*node))->factor) {
	...
	*balanced = 0;
	...
	case character_AVL_RGT_HEAVY:
	  ...
	  *balanced = 1;
      ...
      if (!((character_avlnode_t *)generic_bitree_data(*node))->hidden) {
	...
	if (tree->destroy != NULL) {
	  ...
	  /* tree->destroy(((character_avlnode_t *)generic_bitree_data(*node))->data); */
	}

	((character_avlnode_t *)generic_bitree_data(*node))->data = data;
	((character_avlnode_t *)generic_bitree_data(*node))->hidden = 0;
...
static int generic_hide(character_bistree_t *tree,
			generic_bitreenode_t *node,
			char data)
{
  ...
  if (cmpval < 0) {
   ...
   ((character_avlnode_t *)generic_bitree_data(node))->hidden = 1;
   retval = 0;
  }
...
static bool generic_lookup(character_bistree_t *tree,
			  generic_bitreenode_t *node,
			  char data)
{
  ...
  if (cmpval < 0) {
   /* Move to the left. */
    ...
    if (!((character_avlnode_t *)generic_bitree_data(node))->hidden) {
      ...
      /* *data = ((character_avlnode_t *)generic_bitree_data(node))->data; */
      retval = true;
    }
    else {
      /* Return that the data was not found. */
...
int character_bistree_remove(character_bistree_t *tree,
			   char data)
{
...
bool character_bistree_lookup(character_bistree_t *tree,
			   char data)
{
