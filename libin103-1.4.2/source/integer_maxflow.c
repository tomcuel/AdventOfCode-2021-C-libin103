...
static int maxflow_bfs (integer_graph_t *graph, int source, int target,
			double** localflows, double* capacity, int *parents, int size) {
  ...
  while (integer_queue_size (&queue) > 0) {
	...
	for (; vindex2 != NULL; vindex2 = generic_list_next(vindex2)) {
	  ...
	  if(residual > 0.0 && parents[tempV->vertex] == -1) {
		...
		integer_queue_enqueue (&queue, tempV->vertex);
	      }
	      else {
...
int integer_maxflow(integer_graph_t *graph, int source, int target,
  ...
  *localflows = calloc ((maxIndex+1), sizeof(double*));
  ...
  while (1) {
    ...
    double capacity;
    int* parents = malloc ((maxIndex+1) * sizeof(int));
    if (parents == NULL) {
      /* Should free memory */
      return -1;
    }
